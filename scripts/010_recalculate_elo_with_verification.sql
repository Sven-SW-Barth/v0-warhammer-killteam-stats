-- Recalculate all Elo ratings from scratch with verification
-- This script includes error handling and verification output
...
BEGIN
    ...
    RAISE NOTICE 'Total games to process: %', total_games;
    ...
    LOOP
        ...
        -- Handle NULL Elo ratings (shouldn't happen after reset, but just in case)
        IF player1_elo IS NULL THEN player1_elo := 1200; END IF;
        IF player2_elo IS NULL THEN player2_elo := 1200; END IF;
        ...
        -- Update player Elo ratings in players table
        ...
        IF games_processed % 10 = 0 THEN
            RAISE NOTICE 'Processed % / % games...', games_processed, total_games;
        END IF;
        
    END LOOP;
    
    RAISE NOTICE '========================================';
    RAISE NOTICE 'Elo recalculation completed successfully!';
    RAISE NOTICE 'Total games processed: %', games_processed;
    RAISE NOTICE '========================================';
    
    -- Verification: Show top 10 players by Elo
    RAISE NOTICE 'Top 10 players by Elo rating:';
    FOR game_record IN 
        SELECT playertag, elo_rating, games_played 
        FROM players 
        ORDER BY elo_rating DESC 
        LIMIT 10
    LOOP
        RAISE NOTICE '  % - Elo: % (% games)', game_record.playertag, game_record.elo_rating, game_record.games_played;
    END LOOP;
